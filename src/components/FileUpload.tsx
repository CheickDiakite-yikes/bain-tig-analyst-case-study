...
export default function FileUpload({ dealId, user, files}: { dealId: string, user: User, files: any[]}) {
 ...
 <div key={file.id} className="flex items-start gap-4 p-4 border-2 border-black bg-white hover:bg-gray-50 transition-all group relative">
...
>
 ...
 <Button variant="ghost" size="icon" className="h-8 w-8 border-2 border-black" onClick={() => window.open(file.url,'_blank')}>
 <Download className="w-4 h-4" />
 </Button>
 <Button variant="ghost" size="icon" className="h-8 w-8 border-2 border-[#CC0000] text-[#CC0000] hover:bg-red-50" onClick={(e) => { e.stopPropagation(); setFileToDelete(file);}}>
 ...
 {error && (
 <div className="p-3 bg-red-100 border-2 border-[#CC0000] text-[#CC0000] font-bold uppercase tracking-wider text-sm shrink-0">
 {error}
 </div>
 )}
 ...
 <div 
 {...getRootProps()} 
 className={`border-2 border-dashed border-black p-4 md:p-6 text-center cursor-pointer transition-all  shrink-0 ${
...
>
 ...
 {files.length === 0 ? (
 <div className="text-center py-8 font-bold uppercase tracking-wider text-gray-500 border-2 border-dashed border-black bg-white">
...
>
 ...
 <Button variant="outline" onClick={() => setFileToDelete(null)} className="border-2 border-black">
 Cancel
 </Button>
 <Button onClick={confirmDelete} className="bg-[#CC0000] hover:bg-red-700 text-white border-2 border-black">
 Delete
 </Button>
 </div>
 </div>
