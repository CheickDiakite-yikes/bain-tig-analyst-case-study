...
export default function FileUpload({ dealId, user, files }: { dealId: string, user: User, files: any[] }) {
  ...
  return (
    <div className="p-6 h-full flex flex-col space-y-6 overflow-hidden bg-white">
      <div className="flex items-center justify-between border-b-2 border-black pb-4">
        <h2 className="text-lg font-bold uppercase tracking-wider text-black">Data Room</h2>
        <p className="text-sm font-bold uppercase tracking-wider text-gray-500">Upload CIMs, financial models, and technical architecture docs.</p>
      </div>

      {error && (
        <div className="p-3 bg-red-100 border-2 border-[#CC0000] text-[#CC0000] font-bold uppercase tracking-wider text-sm shadow-[2px_2px_0px_0px_rgba(204,0,0,1)]">
          {error}
        </div>
      )}
      ...
      <div 
        {...getRootProps()} 
        className={`border-2 border-dashed border-black p-8 text-center cursor-pointer transition-all shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] hover:translate-y-[2px] hover:translate-x-[2px] hover:shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] ${
          isDragActive ? 'bg-red-50' : 'bg-white hover:bg-gray-50'
        }`}
      >
        <input {...getInputProps()} />
        <UploadCloud className={`w-12 h-12 mx-auto mb-4 ${isDragActive ? 'text-[#CC0000]' : 'text-black'}`} />
        {isDragActive ? (
          <p className="text-[#CC0000] font-bold uppercase tracking-wider">Drop the files here ...</p>
        ) : (
          <div>
            <p className="text-black font-bold uppercase tracking-wider mb-1">Drag & drop files here, or click to select files</p>
            <p className="text-sm font-bold uppercase tracking-wider text-gray-500">Supports PDF, PPTX, XLSX, DOCX up to 50MB</p>
          </div>
        )}
        {uploading && <p className="text-[#CC0000] font-bold uppercase tracking-wider mt-4 animate-pulse">Uploading files...</p>}
      </div>
      ...
      <div className="flex-1 overflow-auto">
        ...
        {files.length === 0 ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {files.map(file => (
              <div key={file.id} className="flex items-start gap-4 p-4 border-2 border-black bg-white shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] hover:translate-y-[2px] hover:translate-x-[2px] hover:shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] transition-all group">
